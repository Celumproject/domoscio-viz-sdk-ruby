...
describe DomoscioViz::PathRule do
  ...
  created_tag = DomoscioViz::Tag.create({name: "Exercice"})
  created_tag_2 = DomoscioViz::Tag.create({name: "Difficulté 1"})
  ...
  describe 'CREATE' do
    it 'creates the new rule components taggings to the company' do
      created_rule_input_tagging = DomoscioViz::Tagging.create({tag_id: created_tag["id"], taggable_id: created_rule_input["id"], taggable_type: "RuleInput"})
      created_rule_output_tagging = DomoscioViz::Tagging.create({tag_id: created_tag["id"], taggable_id: created_rule_output["id"], taggable_type: "RuleOutput"})
      ...
      created_rule_input_tag = DomoscioViz::Tag.fetch(created_rule_input_tagging["tag_id"])
      created_rule_output_tag = DomoscioViz::Tag.fetch(created_rule_output_tagging["tag_id"])
  ...
  describe 'CREATE' do
    it 'creates the new knowledge_nodes and taggings to the company' do
      ...
      created_tagging_source = DomoscioViz::Tagging.create({tag_id: created_tag["id"], taggable_id: created_knowledge_node_source["id"], taggable_type: "KnowledgeNode"})
      created_tagging_destination = DomoscioViz::Tagging.create({tag_id: created_tag["id"], taggable_id: created_knowledge_node_destination["id"], taggable_type: "KnowledgeNode"})
    end
  end
  ...
  describe 'CREATE' do
    it 'creates the new students and knowledge_node_students to the company' do
      ...
      created_knowledge_node_student = DomoscioViz::KnowledgeNodeStudent.create({
      student_id: new_student["id"],
      knowledge_node_id: created_knowledge_node_source["id"]
    })
      created_result_success = DomoscioViz::Event.create({
  ...
  describe 'Fectch REcommandation' do
    it 'checks if the new recommendantion as been properly created' do
      ...
      recommendations = DomoscioViz::Recommendation.fetch()
      ...
      DomoscioViz::Recommendation.destroy(rec["id"])
  ...
  describe 'Complex Recommendation' do
    it 'creates more complex taggings and several knowledge nodes to prioritize recommandation' do
      ...
      created_tagging_destination_2_1 = DomoscioViz::Tagging.create({tag_id: created_tag["id"], taggable_id: created_knowledge_node_destination_2["id"], taggable_type: "KnowledgeNode"})
      created_tagging_destination_2_2 = DomoscioViz::Tagging.create({tag_id: created_tag_2["id"], taggable_id: created_knowledge_node_destination_2["id"], taggable_type: "KnowledgeNode"})

      created_result_success_2 = DomoscioViz::Event.create({
      :knowledge_node_student_id => created_knowledge_node_student["id"],
      :type => "EventResult",
      :payload => "100"
    })
  ...
  describe 'Fectch REcommandation' do
    it 'checks if the new recommendantion as been properly created' do
      ...
      recommendations.each do |rec|
        ...
        puts DomoscioViz::KnowledgeNode.fetch
  ...
  describe 'DESTROY' do
    it 'destroys all the related and created objects' do
      ...
      DomoscioViz::Tag.destroy(created_tag["id"])
      DomoscioViz::KnowledgeNode.destroy(created_knowledge_node_source["id"])      
      DomoscioViz::KnowledgeNode.destroy(created_knowledge_node_destination["id"]) 
      DomoscioViz::KnowledgeNodeStudent.destroy(created_knowledge_node_student["id"])
      DomoscioViz::Student.destroy(created_student["id"])
      DomoscioViz::Event.destroy(created_result_success["id"])
  ...
  describe 'DESTROY' do
    it 'checks all the objects have properly been destroyed' do
      ...
      expect(DomoscioViz::Tag.fetch).to be_empty
      #expect(DomoscioViz::KnowledgeNode.fetch).to be_empty
      expect(DomoscioViz::Recommendation.fetch).to be_empty
      #expect(DomoscioViz::KnowledgeNodeStudent.fetch).to be_empty
      #expect(DomoscioViz::Student.fetch).to be_empty
      #expect(DomoscioViz::Event.fetch).to be_empty
